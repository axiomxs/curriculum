...
export const HaloAnimation: React.FC<SizeProps> = ({ size }) => {
  {
    /*创建状态 mouse 用于存储鼠标坐标 */
  }
  ...
  {
    /*
    使用React.useEffect返回值中的ref.current以确保获取到实际DOM元素
    是存储位置信息的 DOMRect 对象
    */
  }
  const svgPosInfo = React.useRef<DOMRect | null>(null);
  useEffect(() => {
    if (ref.current) {
      svgPosInfo.current = ref.current.getBoundingClientRect();
    }
  }, [ref]);
  {
    /*从 DOMRect 对象中提取几何属性 */
  }
  const svgRect = svgPosInfo.current;
  const svgLeft = svgRect?.left ?? 0;
  const svgTop = svgRect?.top ?? 0;
  const svgWidth = svgRect?.width ?? 0;
  const svgHeight = svgRect?.height ?? 0;
